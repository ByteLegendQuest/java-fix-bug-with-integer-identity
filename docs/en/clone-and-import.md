...
`git clone https://github.com/ByteLegendQuest/java-fix-bug-with-integer-identity`

A project directory named `java-fix-bug-with-integer-identity` will be created, which can be moved wherever you like.

Open `IDEA`, `File` - `Open`, select and open the `pom.xml` in `java-fix-bug-with-integer-identity` directory which is created just now.
